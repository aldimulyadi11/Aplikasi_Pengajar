...
<body>
  <div id="login-page">
    <div class="container">
      <form class="form-login" method="POST" action="{{ route('user.tambahMateriStore', $id) }}">
        @csrf
        @include('layouts.app.message')
        <h2 class="form-login-heading">Input Password</h2>
        <div class="login-wrap">          
          <input type="password" class="form-control" name="password" placeholder="New Password">
          <br><br>
          <button class="btn btn-theme btn-block" type="submit"><i class="fa fa-lock"></i> CONFIR</button>
          <hr>          
        </div>
      </form>
    </div>
